...
# Create and mount the outputs directory so images can be served to the frontend
...
async def analyze_image(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Convert internal disk paths to public URLs for the frontend
        saved_urls = {}
        if result.get("saved_paths"):
            paths = result["saved_paths"]
            if "overlay_path" in paths:
                saved_urls["overlay_url"] = f"/outputs/overlays/{os.path.basename(paths['overlay_path'])}"
            if "vessel_mask_path" in paths:
                saved_urls["vessel_mask_url"] = f"/outputs/masks/{os.path.basename(paths['vessel_mask_path'])}"
            if "stenosis_mask_path" in paths:
                saved_urls["stenosis_mask_url"] = f"/outputs/masks/{os.path.basename(paths['stenosis_mask_path'])}"
    ...
    try:
        ...
        if success and os.path.exists(output_video_path):
            return JSONResponse(status_code=200, content={
                "status": "success",
                "data": {
                    "video_url": f"/download/{unique_id}.mp4",
...
async def download_file(filename: str):
    # Secure filename extraction to prevent directory traversal
